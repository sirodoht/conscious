...
def vote(request, statement_id, value):
    ...
    participant = models.Participant.objects.create(public_id="a" + str(randint(100, 999)))
...
def api_vote(request, conversation_id):
    ...
    if form.is_valid():
        ...
        if vote_value not in ("agree", "disagree", "pass"):
            return JsonResponse({
                "error": {"vote": ["This field can only be 'agree', 'disagree', 'pass'."]},
            })

        conversation = get_object_or_404(models.Conversation, id=conversation_id)
        statement = models.Statement.objects.filter(conversation=conversation).latest("created_at")
        participant, _ = models.Participant.objects.get_or_create(public_id=participant_public_id)
        vote = models.Vote.objects.create(
            value=vote_value,
            statement=statement,
            participant=participant,
